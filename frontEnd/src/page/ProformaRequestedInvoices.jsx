...
export default function ProformaRequestedInvoices() {
    ...
    if (
      ...
      if (q.quotationCategory === "Normal Quotation") {
        ...
        autoTable(doc, {
          ...
          body: [
            [
              `Recived ${q.advanceRequestedAmountPercent}% Advance Payment (Without VAT) `,
              "Rs.",
              `${q.advanceRequestedAmount.toLocaleString(undefined, {
                minimumFractionDigits: 2,
              })}`,
        ...
        doc.text("Total Amount in Words:", 13, finalY);
        doc.setFont("helvetica", "normal");
        const totalInWordsAdvance = numberToWords(
          Math.floor(q.advanceRequestedAmount),
        );
        doc.text(`${totalInWordsAdvance}`, 55, finalY);
        finalY += 8;
        ...
        autoTable(doc, {
          ...
          body: [
            [
              `Requesting Balance Payment (With VAT) `,
              "Rs.",
              `${q.RequestingBalancePayment.toLocaleString(undefined, {
                minimumFractionDigits: 2,
              })}`,
            ],
          ],
        ...
        doc.text("Total Amount in Words:", 13, finalY);
        doc.setFont("helvetica", "normal");
        const totalInWordsBalance = numberToWords(
          Math.floor(q.RequestingBalancePayment),
        );
        doc.text(`${totalInWordsBalance}`, 55, finalY);
        finalY += 12;
  ...
  const stats = {
    ...
    proformaRequested: allInvoices.filter(
      (inv) =>
        inv.proformaCategory === "Requesting Advance" &&
        !inv.RequestingBalancePayment,
    ).length,
    RequestingBalancePayment: allInvoices.filter(
      (inv) =>
        inv.proformaCategory === "Requesting Advance" &&
        inv.RequestingBalancePayment,
    ).length,
  ...
  return (
    <div className="dashboard-container" style={{ padding: "20px" }}>
      ...
      >
        <div className="stat-card blue" style={{ backgroundColor: "#f4b9bb" }}>
        ...
        <div className="stat-card blue" style={{ backgroundColor: "#f4b9bb" }}>
          <h3>Proforma Requesting Balance Payment Invoices</h3>
          <p className="value">
            {stats.RequestingBalancePayment.toString().padStart(2, "0")}
          </p>
        </div>
      </div>
        ...
        >
          ...
          <tbody>
            ...
            {allInvoices.filter(
              ...
              allInvoices
                .filter((inv) => inv.proformaCategory === "Requesting Advance") // පෙරා ගැනීම
                .map((inv, index) => (
                  <tr key={index}>
                    <td>{inv.date}</td>
                    <td>{inv.invoiceNo}</td>
                    ...
                    <td>
                      <button
                        className="btn-edit"
                        // 🟢 ID එක ලිස්ට් එකේ තියෙනවා නම් බොත්තම Disable වේ
                        disabled={
                          generatedIds.includes(inv._id) ||
                          inv.RequestingBalancePaymentNo
                        }
                        onClick={async (e) => {
                          e.stopPropagation();
                          const advancePaid =
                            Number(inv.advanceRequestedAmount) || 0;
                          let baseAmount = 0;
                          let balanceToPay = 0;
                          // --- 🟢 Base Amount Logic (ඔයාගේ පරණ logic එක එලෙසම) ---
                          if (
                            (inv.discountButtonYes === 0 ||
                              inv.discountButtonYes === "0") &&
                            (inv.multiDays === 0 || inv.multiDays === "0")
                          ) {
                            if (inv.quotationCategory === "Normal Quotation") {
                              baseAmount = inv.subTotal;
                              balanceToPay = baseAmount - advancePaid;
                            } else if (
                              inv.quotationCategory === "Vat Quotation"
                            ) {
                              baseAmount = inv.subTotal;
                              balanceToPay =
                                baseAmount - advancePaid + baseAmount * 0.18;
                            } else if (
                              inv.quotationCategory ===
                              "Normal Quotation with Discount"
                            ) {
                              baseAmount = inv.subTotal - inv.discount;
                              balanceToPay = baseAmount - advancePaid;
                            } else if (
                              inv.quotationCategory ===
                              "Vat Quotation with Discount"
                            ) {
                              baseAmount = inv.totalValueOfSupply;
                              balanceToPay =
                                baseAmount - advancePaid + baseAmount * 0.18;
                            }
                          } else if (
                            (inv.discountButtonYes === 1 ||
                              inv.discountButtonYes === "1") &&
                            (inv.multiDays === 0 || inv.multiDays === "0")
                          ) {
                            if (inv.quotationCategory === "Normal Quotation") {
                            ...
                            const eventTotal = (inv.eventDate || []).reduce(
                              (sum, item) => sum + (Number(item.amount) || 0),
                              0,
                            );
                            const ledGrandSum =
                              eventTotal + (Number(inv.rehearsalAmount) || 0);
                            const otherSystemsTotal =
                              (Number(inv.subTotal) || 0) -
                              (Number(inv.totalLEDLightingSoundStageTruss) ||
                                0);
                            baseAmount =
                              ledGrandSum -
                          ...
                          // 1. තහවුරු කිරීමේ Pop-up එක
                          const confirmResult = await Swal.fire({
                            title: "Do you want to generate the Tax Invoice?",
                            html: `
      <div style="margin-top: 10px; padding: 15px; background: #fff5f5; border: 1px solid #feb2b2; border-radius: 8px; text-align: left;">
        <p style="margin: 0 0 5px 0; font-size: 13px; color: #4a5568;">Total Base Amount: <b>Rs. ${baseAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</b></p>
        <p style="margin: 0 0 5px 0; font-size: 13px; color: #e53e3e;">Advance Paid: <b>Rs. ${advancePaid.toLocaleString(undefined, { minimumFractionDigits: 2 })}</b></p>
        <hr style="border: 0; border-top: 1px solid #feb2b2; margin: 10px 0;">
        <p style="margin: 0; color: #c53030; font-weight: bold; font-size: 15px;">
          Requesting Balance Payment : Rs. ${balanceToPay.toLocaleString(undefined, { minimumFractionDigits: 2 })}
        </p>
      </div>
      <p style="margin-top: 15px; font-size: 14px; color: #718096;">Once confirmed, you can proceed to assign an invoice number.</p>
    `,
                            icon: "question",
                          ...
                          // 2. User "Yes" කළොත් පමණක් Invoice Number එක අහනවා
                          if (confirmResult.isConfirmed) {
                            const { value: formValues } = await Swal.fire({
                              title: "Enter Invoice Details",
                              html: `
        <input id="swal-invoice-no" class="swal2-input" placeholder="Invoice Number (e.g. INV-001)">
      `,
                              focusConfirm: false,
                              preConfirm: () => {
                                const no =
                            ...
                            // 3. Database එක Update කිරීම (Axios)
                            if (formValues) {
                              try {
                                const response = await axios.put(
                                  `https://imagine-entertainment-invoice-system.onrender.com/api/quotations/${inv._id}`,
                                  {
                                    ...inv,
                                    RequestingBalancePaymentNo: formValues, // Invoice Number එක
                                    invoiceType:
                                      "Requesting Balance Payment Invoice",
                                    RequestingBalancePayment: balanceToPay, // 🟢 මෙතනදී Balance එක Database එකට යවනවා
                                  },
                                );

                                if (response.status === 200) {
                                  setGeneratedIds((prev) => [...prev, inv._id]);
                                  setAllInvoices((prevInvoices) =>
                                    prevInvoices.map((item) =>
                                      item._id === inv._id
                                        ? {
                                            ...item,
                                            RequestingBalancePaymentNo:
                                              formValues,
                                            invoiceType:
                                              "Requesting Balance Payment Invoice",
                                          }
                                  ...
                                  Swal.fire({
                                    title: "Saved!",
                                    text: "Tax Invoice and Balance Payment saved successfully.",
                                    icon: "success",
                                    timer: 2000,
                        ...
                        style={{
                          backgroundColor: generatedIds.includes(inv._id)
                            ? "#cbd5e0"
                            : "#3182ce",
                          color:
                            generatedIds.includes(inv._id) ||
                      ...
                      >
                        {/* 🟢 ID එක ලිස්ට් එකේ තිබේ නම් නම මාරු වේ */}
                        {generatedIds.includes(inv._id) ||
                        !!inv.RequestingBalancePaymentNo
                          ? "Tax Invoice Created"
                          : "Go to Tax Invoice"}
