...
export default function ViewQuotation() {
  ...
  const generateNormalPDF = (q, actionType) => {
    ...
    if (q.quotationCategory === "Normal") {
    ...
    } else if (q.quotationCategory === "Non-Tax with Discount") {
      autoTable(doc, {
        startY: finalY,
        body: [
          [
      ...
      doc.text(`${totalInWords}`, 55, finalY);
    } else if (q.quotationCategory === "Non-Tax without Discount") {
      autoTable(doc, {
        startY: finalY,
        body: [
          [
  ...
  const generateSummaryPDF = (q, actionType) => {
    ...
    if (hasPowerItems) {
      ...
      if (powerSumData && powerSumData.row && powerSumData.row.length > 0) {
        autoTable(doc, {
          startY: finalY,
          body: [
            [
    ...
    // 1. මුලින්ම category එකේ items තිබේදැයි බලන්න
    const hasOtherItems =
      q.otherSystems &&
      q.otherSystems.some((sub) => sub.lineItems && sub.lineItems.length > 0);

    if (hasOtherItems) {
      // Items තිබේ නම් පමණක් Category Table එක අඳින්න
      OSystems.forEach((cat) => renderCategoryTable(cat));

      // Summary Details වලින් දත්ත සොයා ගැනීම
      const otherSumData = q.summaryDetails?.find(
        (d) => d.categoryId === "other",
      );

      // 2. Summary Breakdown වගුව පෙන්විය යුත්තේ items සහ summary data යන දෙකම තිබේ නම් පමණි
      if (otherSumData && otherSumData.row && otherSumData.row.length > 0) {
        autoTable(doc, {
          ...
          body: [
            ...
            [
              {
                content: `Total Amount for Rehearsal day and Event days.`,
                colSpan: 3,
                styles: {
                  halign: "left",
                  fontStyle: "bold",
                ...
                <td>
                  <button
                    ...
                    onClick={async (e) => {
                      ...
                      // 1. Pop-up පෙන්වීම
                      const { value: typedInvoiceNo } = await Swal.fire({
                        title: "Enter Invoice Number",
                        input: "text",
                        inputLabel: `Enter the Invoice number for ${q.invoiceNo}`,
                        inputPlaceholder: "e.g. INV-2026-001",
                        showCancelButton: true,
                        confirmButtonText: "Create & Save",
                        inputValidator: (value) => {
                          if (!value) {
                            return "It is mandatory to enter the invoice number!";
                          }
                        },
                      });
                        ...
                        try {
                          ...
                          const response = await axios.put(
                            `https://imagine-entertainment-invoice-system.onrender.com/api/quotations/${q._id}`,
                            {
                              ...
                              invoiceFooterType: "VAT All",
                            },
                          );

                          if (response.status === 200) {
                            // සාර්ථක නම් දත්ත fetchQuotations() කර නැවත ලැයිස්තුව refresh කරන්න
                            if (typeof fetchQuotations === "function")
                              fetchQuotations();

                            Swal.fire({
                              title: "Invoice Generated!",
                              text: `Invoice ${typedInvoiceNo} has been saved successfully.`,
                              icon: "success",
                              timer: 2000,
