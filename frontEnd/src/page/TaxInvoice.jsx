    ...
    <div className="dashboard-container" style={{ padding: "20px" }}>
      ...
      <div className="table-scroll-container">
        ...
        >
          ...
          <tbody>
            {/* 🟢 මෙතනදී .filter() එක භාවිතා කරලා Tax Invoice විතරක් තෝරාගන්නවා */}
            {allInvoices.filter((inv) => inv.invoiceType === "Tax Invoice")
              .length > 0 ? (
              allInvoices
                .filter((inv) => inv.invoiceType === "Tax Invoice") // පෙරා ගැනීම
                .map((inv, index) => (
                  <tr key={index}>
                    <td>{inv.date}</td>
                    <td>{inv.invoiceNo}</td>
